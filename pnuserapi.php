...
function mediashare_userapi_getAlbumData($args)
{
    ...
    if ($album['mainMediaId'] > 0) {
        $mainMediaItem = pnModAPIFunc('mediashare', 'user', 'getMediaItem', array('mediaId' => $album['mainMediaId']));
...
function mediashare_userapi_getAlbumList($args)
{
    ...
    $accessibleAlbumSql = pnModAPIFunc('mediashare', 'user', 'getAccessibleAlbumsSql', array('access' => $access, 'field' => $albumsColumn['id']));
    if ($accessibleAlbumSql === false) {
        return false;
    }
    $sql = "SELECT   album.$albumsColumn[id],
...
function mediashare_userapi_getRandomMediaItem($args)
{
    ...
    $accessibleAlbumSql = pnModAPIFunc('mediashare', 'user', 'getAccessibleAlbumsSql', array('access' => mediashareAccessRequirementView, 'field' => "album.$albumsColumn[id]"));
    if ($accessibleAlbumSql === false) {
        return false;
    }
...
function mediashare_userapi_getMostActiveUsers($args)
{
    ...
    for (; !$dbresult->EOF; $dbresult->MoveNext()) {
        $active = array('uname' => $dbresult->fields[0], 'count' => $dbresult->fields[1]);
...
function mediashare_userapi_getSummary($args)
{
    ...
    $accessibleAlbumSql = pnModAPIFunc('mediashare', 'user', 'getAccessibleAlbumsSql', array('access' => mediashareAccessRequirementViewSomething, 'field' => "$mediaColumn[parentAlbumId]"));
    if ($accessibleAlbumSql === false)
        return false;
    ...
    $accessibleAlbumSql = pnModAPIFunc('mediashare', 'user', 'getAccessibleAlbumsSql', array('access' => mediashareAccessRequirementViewSomething, 'field' => "$albumsColumn[id]"));
    if ($accessibleAlbumSql === false)
        return false;
