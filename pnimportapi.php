...
{
  $folders =  pnModAPIFunc('photoshare', 'user', 'get_accessible_folders',
                           array('getForList'      => true,
                                 'order'           => 'title',
                                 'parentFolderID'  => $photoshareFolderId) );
  if ($folders === false)
  ...
  foreach ($folders as $folder)
  {
    ...
    {
      $folderData = pnModAPIFunc('photoshare', 'user', 'get_folder_info',
...
{
  $images = pnModAPIFunc('photoshare', 'user', 'get_image_list',
                         array('folderID'     => $photoshareFolderId,
                               'usePageCount' => false));
  if ($images === false)
    return mediashareErrorAPI(__FILE__, __LINE__, photoshareErrorAPIGet());
  ...
  foreach ($images['images'] as $image)
  {
    ...
    {
      $imageData = pnModAPIFunc('photoshare', 'show', 'get_image_info',
                                array('imageID' => $image['id']));
      if ($imageData === false)
        return mediashareErrorAPI(__FILE__, __LINE__, photoshareErrorAPIGet());
      ...
      $args = array('albumId'          => $mediashareAlbumId,
