...
function mediashare_invitation_send($args)
{
  ...
  else
  {
    ...
    $invitation = array('email'     => '',
                        'subject'   => _MSINVSUBJECTTEXT,
                        'text'      => '',
                        'sender'    => pnUserGetVar('uname'),
                        'expires'   => '');
  }
...
{
  $input = array('albumId' => pnVarCleanFromInput('albumid'),
...
{
  $args = array('invitationId' => $invitationId,
                'emails' => pnVarCleanFromInput('emails'),
                'subject' => pnVarCleanFromInput('subject'),
                'text' => pnVarCleanFromInput('text'),
                'sender' => pnVarCleanFromInput('sender'),
...
function mediashare_invitation_list($args)
{
  ...
  $invitations = pnModAPIFunc('mediashare', 'invitation', 'getInvitations',
                              array('albumId' => $albumId));
...
function mediashareExpireInvitations($args)
{
  ...
  if (!empty($invitations))
  {
    ...
    $ok = pnModAPIFunc('mediashare', 'invitation', 'expireInvitations', 
                       array('albumId' => $albumId,
                             'expires' => $expires,
...
function mediashareDeleteInvitations($args)
{
  ...
  if (!empty($invitations))
  {
    ...
    $ok = pnModAPIFunc('mediashare', 'invitation', 'deleteInvitations', 
                       array('albumId' => $albumId,
                             'invitations' => $invitations));
    if ($ok === false)
      return mediashareErrorAPIGet();
...
function mediashare_invitation_open($args)
{
  ...
  $result = pnModAPIFunc('mediashare', 'invitation', 'register',
                         array('key' => $key));
  if ($result === false)
    return mediashareErrorAPIGet();
  else if (!$result['ok'])
